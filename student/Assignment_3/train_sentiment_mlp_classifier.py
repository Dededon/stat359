...
def mean_pool_sentence(sentence):
...
class MLPTensorDataset(Dataset):
...
def evaluate_on_test(model, test_loader, device, model_path):
    ...
    plot_confusion_matrix(cm, class_names, "outputs/mlp_confusion_matrix.png")
...
if __name__ == "__main__":
    ...
    log_file = open(os.path.join("outputs", "mlp_run.log"), "w", encoding="utf-8")
    ...
    X_train = np.vstack([mean_pool_sentence(s) for s in X_train_text])
    X_val = np.vstack([mean_pool_sentence(s) for s in X_val_text])
    X_test = np.vstack([mean_pool_sentence(s) for s in X_test_text])
    ...
    train_dataset = MLPTensorDataset(X_train, y_train)
    val_dataset = MLPTensorDataset(X_val, y_val)
    test_dataset = MLPTensorDataset(X_test, y_test)
    ...
    evaluate_on_test(model, test_loader, device, "outputs/best_mlp_model.pth")
